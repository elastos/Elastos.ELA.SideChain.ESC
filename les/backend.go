...
func New(ctx *node.ServiceContext, config *eth.Config, node *node.Node) (*LightEthereum, error) {
	...
	if leth.protocolManager, err = NewProtocolManager(leth.chainConfig, light.DefaultClientIndexerConfig, true, config.NetworkId, leth.eventMux, leth.engine, leth.peers, leth.blockchain, nil, chainDb, leth.odr, leth.relay, leth.serverPool, quitSync, &leth.wg, node.Stop); err != nil {
		return nil, err
	}
