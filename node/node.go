...
func (n *Node) Stop() error {
	n.lock.Lock()
	defer n.lock.Unlock()
	fmt.Println("node start stop>>>>> 1")
	// Short circuit if the node's not running
	if n.server == nil {
		return ErrNodeStopped
	}
	fmt.Println("node start stop>>>>> 2 Terminate the API, services and the p2p serve")
	...
	n.services = nil
	n.server = nil
	fmt.Println("node start stop>>>>> 3 Terminate the API, services and the p2p server stoped")
	...
	// unblock n.Wait
	close(n.stop)
	fmt.Println("node start stop>>>>> 4")
	// Remove the keystore if it was created ephemerally.
	var keystoreErr error
	if n.ephemeralKeystore != "" {
		keystoreErr = os.RemoveAll(n.ephemeralKeystore)
	}
	fmt.Println("node start stop>>>>> 5")
	...
	fmt.Println("node start stop>>>>> 6 end")
	return nil
}
