...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	if ptd == nil {
		fmt.Println("writeBlockWithState 111111")
	...
	if err := bc.hc.WriteTd(block.Hash(), block.NumberU64(), externTd); err != nil {
		fmt.Println("writeBlockWithState 2222")
	...
	if !reorg && externTd.Cmp(localTd) == 0 {
		...
		fmt.Println("writeBlockWithState 444444", "block.NumberU64()", block.NumberU64(), "currentBlock.NumberU64() ", currentBlock.NumberU64())
	...
	if reorg {
		...
		if block.ParentHash() != currentBlock.Hash() {
			fmt.Println("reorg 重组", "parent", block.ParentHash().String(), "current", currentBlock.Hash().String())
		...
		fmt.Println("writeBlockWithState 5555 SideStatTy")
		status = SideStatTy
	}
	if err := batch.Write(); err != nil {
		return NonStatTy, err
	...
	if status == CanonStatTy {
		fmt.Println("writeBlockWithState 6666666 ")
		bc.insert(block)
	}
	...
	fmt.Println("writeBlockWithState 7777777 ")
	return status, nil
}
...
func (bc *BlockChain) insertBlockChain(chain types.Blocks, verifySeals bool, engine consensus.Engine) (int, []interface{}, []*types.Log, error) {
	...
	fmt.Println("insertBlockChain 11111", headers[0].Number, "hash", headers[0].Hash().String())
	...
	for ; block != nil && err == nil || err == ErrKnownBlock; block, err = it.next() {
		...
		substart := time.Now()
		fmt.Println("insertBlockChain 222222 Process", headers[0].Number, "hash", headers[0].Hash().String())
		...
		substart = time.Now()
		fmt.Println("insertBlockChain 222222 writeBlockWithState", headers[0].Number, "hash", headers[0].Hash().String())
		status, err := bc.writeBlockWithState(block, receipts, statedb)
		fmt.Println("insertBlockChain 3333 writeBlockWithState", headers[0].Number, "status", status)
		if err != nil {
			atomic.StoreUint32(&followupInterrupt, 1)
			return it.index, events, coalescedLogs, err
		}
